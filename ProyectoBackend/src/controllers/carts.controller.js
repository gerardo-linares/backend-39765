...
export const createCart = async (req, res) => {
  try {
    ...
    const newCart = await CartsManager.addCart({ name, price });
...
export const getCartById = async (req, res) => {
  try {
    ...
    const cart = await CartsManager.getCartById(cartId).populate(
      "products.product"
    );
    if (cart) {
      res.send({
        status: "success",
        message: `El cartito '${req.params.cid}' se ha cargado con exito`,
        payload: cart,
      });
    } else {
      res.status(400).send({
        status: "error",
        error: "Producto no encontrado, ingrese una Id valida",
      });
    }
  } catch (error) {
    console.log(error);
...
export const updateCartById = async (req, res) => {
  try {
    ...
    if (existingProducts.length !== productIds.length) {
      res.status(400).send({
        error:
          "Una o más IDs de productos no existen en la base de datos. Por favor, ingrese IDs válidas",
      });
      return;
    }
...
export const deleteAllProducts = async (req, res) => {
  try {
    ...
    const deleteAllProducts = await CartsManager.deleteAllProducts(
      cartId,
      deleteProducts
    );
...
export const updateProductQuantity = async (req, res) => {
  try {
    ...
    const updatedProductQuantity = await cartManager.updateProductQuantity(
      cid,
      pid,
      quantity
    );
...
export const deleteProductFromCart = async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({
      status: "success",
      message: `El producto '${deleteProduct.title}' ha sido eliminado con exito`,
